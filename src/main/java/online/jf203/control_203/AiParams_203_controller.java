...
//    @Scheduled(fixedRate = 30000)
    public List<String> aicontrol2(@RequestBody List<String> data) {
        ...
        // 根据openAiTemp的值处理AI功能的开启或关闭，并记录日志
    ...
    @ResponseBody
//    @Scheduled(fixedRate = 30000)
    public Map<String,Object> params_post(@RequestBody JSONObject data){// 接收一个Integer类型的请求数据，返回一个Map对象，包含多种控制参数和状态
        LinkedList<Double> coldsite_gap=new LinkedList<>();//存储计算出的冷通道测点温差
...
//        List <Map<String,Object>> list_np=jdbc.queryForList(sql_np);
        ...
        for(Map<String,Object>c:list_ab) {//遍历listab，提取其中名为value0的字段
            Double gap = (Double)c.get("Value0");
            gap_all.add(gap);
        }
        coldsite_gap.add(Collections.max(gap_all));//用collection.max找到温差最大值
...
//        }
    ...
    @CrossOrigin
    @PostMapping("/getData/203/diaoduqi_pwd")
    @ResponseBody
//    @Scheduled(fixedRate = 30000)
    public String diaoduqi_pwd(@RequestBody JSONObject data) {
        ...
        if (pwdTemp.isEmpty() || userName.isEmpty() || userRole.isEmpty() || time_operate.isEmpty()){
            ...
            String sql_insert1 = "insert into log(content, userName, userRole, time, datacenter_room) values('" + diaoduqilog + "','" + userName + "','" + userRole + "','" + time_operate + "','JF203');";
            jdbc.execute(sql_insert1);
            ret="验证成功";
        } else {
            ret="验证失败";
